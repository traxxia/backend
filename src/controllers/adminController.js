...
class AdminController {
  ...
  static async getAuditTrail(req, res) {
    try {
      ...
      if (req.user.role.role_name === 'company_admin') {
        const companyUsers = await UserModel.getAll({ company_id: req.user.company_id });
      ...
      const auditEntries = await AuditModel.find(filter, { skip, limit: parseInt(limit), projection });
  ...
  static async getUserData(req, res) {
    try {
      ...
      if (business_id && ObjectId.isValid(business_id)) {
        ...
        const businessExists = await BusinessModel.findById(businessObjectId, targetUserId);
        if (!businessExists) {
      ...
      const phaseAnalysis = await ConversationModel.findByFilter(phaseAnalysisFilter);
          ...
          businessConversations.forEach(conv => {
            if (conv.question_id) {
              ...
              if (conv.metadata && conv.metadata.is_complete === true) {
                businessQuestionStats[questionId].isComplete =businessQuestionStats[questionId].isComplete = true;
              }
            }
          });
